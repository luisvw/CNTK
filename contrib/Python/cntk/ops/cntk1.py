...
class NewReshape(ComputationNode):
    def __init__(self, input, dims, beginAxis=0, endAxis=0, name='NewReshape', var_name=None):
        super(NewReshape, self).__init__(params=['input', 'dims', 'beginAxis', 'endAxis'], name=name, var_name=var_name)
        self.input = input
        self.dims = dims
...
class TransposeDimensions(ComputationNode):
    def __init__(self, input, axis1, axis2, name='TransposeDimensions', var_name=None):
        super(TransposeDimensions, self).__init__(params=['input', 'axis1', 'axis2'], name=name, var_name=var_name)
        self.input = input
        self.axis1 = axis1
        self.axis2 = axis2
        self.params_with_defaults = []
...
class Convolution(ComputationNode):
    def __init__(self, weightNode, inputValueNode, kernelDims, mapDims=1, stride=1, sharing=True, autoPadding=True, lowerPad=0, upperPad=0, imageLayout='CHW', maxTempMemSizeInSamples=0, name='Convolution', var_name=None):
        super(Convolution, self).__init__(params=['weightNode', 'inputValueNode', 'kernelDims', 'mapDims', 'stride', 'sharing', 'autoPadding', 'lowerPad', 'upperPad', 'imageLayout', 'maxTempMemSizeInSamples'], name=name, var_name=var_name)
...
class Pooling(ComputationNode):
    def __init__(self, input, poolKind, kernelDims, stride=1, autoPadding=True, lowerPad=0, upperPad=0, imageLayout='CHW', name='Pooling', var_name=None):
        super(Pooling, self).__init__(params=['input', 'poolKind', 'kernelDims', 'stride', 'autoPadding', 'lowerPad', 'upperPad', 'imageLayout'], name=name, var_name=var_name)
        self.input = input
...
class BatchNormalization(ComputationNode):
    def __init__(self, input, scale, bias, runMean, runInvStdDev, spatial, normalizationTimeConstant=0, blendTimeConstant=0, epsilon=1e-05, useCntkEngine=True, imageLayout='CHW', name='BatchNormalization', var_name=None):
        super(BatchNormalization, self).__init__(params=['input', 'scale', 'bias', 'runMean', 'runInvStdDev', 'spatial', 'normalizationTimeConstant', 'blendTimeConstant', 'epsilon', 'useCntkEngine', 'imageLayout'], name=name, var_name=var_name)
        self.input = input
        self.scale = scale
        self.bias = bias
...
class If(ComputationNode):
    def __init__(self, cond, thenVal, elseVal, name='If', var_name=None):
        super(If, self).__init__(params=['cond', 'thenVal', 'elseVal'], name=name, var_name=var_name)
        self.cond = cond
...
class RowSlice(Slice):
    def __init__(self, beginIndex, numRows, input, name='RowSlice', var_name=None):
        super(RowSlice, self).__init__(beginIndex, beginIndex + numRows, input, axis=1, name=name, var_name=var_name)
...
class ReshapeDimension(NewReshape):
    def __init__(self, x, axis, tensorShape, name='ReshapeDimension', var_name=None):
        super(ReshapeDimension, self).__init__(x, tensorShape, beginAxis=axis, endAxis=axis + 1, name=name, var_name=var_name)
...
class FlattenDimensions(NewReshape):
    def __init__(self, x, axis, num, name='FlattenDimensions', var_name=None):
        super(FlattenDimensions, self).__init__(x, 0, beginAxis=axis, endAxis=axis + num, name=name, var_name=var_name)
        self.params=['x', 'axis', 'num']
        self.params_with_defaults = []

class SplitDimension(ReshapeDimension):
    def __init__(self, x, axis, N, name='SplitDimension', var_name=None):
        super(SplitDimension, self).__init__(x, axis, '<not yet supported>', name=name, var_name=var_name)
